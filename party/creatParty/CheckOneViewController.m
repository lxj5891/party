...
- (void)viewDidLoad
{
    ...
    self.choiceFriends=[[NSMutableArray alloc]init];
    if(self.spot==2)
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (dataFlag==2) {
        ...
        self.playList=[[NSMutableArray alloc]initWithArray:[bizDic objectForKey:@"users"]];
        [self loadFridetail];
    }else if(dataFlag==1){
        NSData* response=[request responseData];
        //NSLog(@"%@",response);
        NSError* error;
        NSDictionary* bizDic=[NSJSONSerialization JSONObjectWithData:response options:NSJSONReadingMutableLeaves error:&error];
        self.list=[[NSMutableArray alloc]initWithArray:[bizDic objectForKey:@"users"]];

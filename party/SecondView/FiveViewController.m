...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==3) {
        ...
        if (indexPath.row==1) {
            ...
            UILabel *labelNameP_INFO = [[UILabel alloc] initWithFrame:CGRectZero];
            labelNameP_INFO.numberOfLines = 0;
            labelNameP_INFO.font=[UIFont systemFontOfSize:14];
            labelNameP_INFO.backgroundColor=[UIColor clearColor];
            labelNameP_INFO.textColor=[UIColor colorWithRed:99.0/255 green:99.0/255 blue:99.0/255 alpha:1];
            [cell.contentView addSubview:label];
            
            
            CGRect cellFrame = CGRectMake(12, 10.0, 280, 30);
            labelNameP_INFO.text=[party objectForKey:@"P_INFO"];
            CGRect rect = cellFrame;
            labelNameP_INFO.frame = rect;
            [labelNameP_INFO sizeToFit];
            cellFrame.size.height = label.frame.size.height+10;
            [labelNameP_INFO setFrame:cellFrame];
            [labelNameP_INFO release];
        }
        if (indexPath.row==2) {
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section==0) {
        UIView* view=[[[UIView alloc]initWithFrame:CGRectMake(0,100, 320, 120)]autorelease];
        ...
        UIButton* personButtonUnin=[UIButton buttonWithType:UIButtonTypeRoundedRect];
